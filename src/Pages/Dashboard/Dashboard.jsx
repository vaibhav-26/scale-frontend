...
function Dashboard() {
          ...
          <span style={{ display: "flex", justifyContent: "center" }}>
            {record?.cvUrl ? (
              <Typography.Link
                ...
                target="_blank"
                href={process.env.REACT_APP_URL + record.cvUrl}
              >
                <DownloadOutlined />
              </Typography.Link>
            ) : (
  ...
  const onFinish = async (values) => {
    ...
    editID
      ? await putRequest(`candidate/edit/${editID}`, candidateData)
          .then(({ data }) => {
            setIsModalOpen(false);
            setCandidates((candidates) =>
      ...
      : await postRequest(`candidate/add`, candidateData)
          .then(({ data }) => {
            setIsModalOpen(false);
      ...
      >
        <Form
          ...
          initialValues={{
            ...
            dob: moment(editData?.dob) || undefined,
            technology: editData?.technology,
            relevantPosition:
              (editData && editData?.Position && editData?.Position[0]?._id) ||
              undefined,
